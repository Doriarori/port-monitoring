...
JWT-авторизация с пользователями в БД (bcrypt-хэши паролей) и двумя ролями.
Авторизация включена по умолчанию — API защищён токеном, в UI открывается
страница входа.

**Bootstrap-админ.** При первом старте, если таблица `users` пуста,
автоматически создаётся админ из `ADMIN_USERNAME`/`ADMIN_PASSWORD`. Эти env-креды
также всегда принимаются как резервный супер-админ — защита от полной блокировки,
если в БД случайно не осталось активных админов.

```bash
# 1. Создать .env из примера
cp .env.example .env

# 2. Задать пароль bootstrap-админа и секрет для подписи токенов
...
| `ADMIN_USERNAME`     | `admin`      | Имя bootstrap-админа                              |
| `ADMIN_PASSWORD`     | —            | Пароль bootstrap-админа (обязателен, если auth вкл)|
| `JWT_SECRET`         | —            | Секрет подписи JWT (обязателен, если auth включён)|
| `JWT_EXPIRE_MINUTES` | `720`        | Время жизни токена в минутах (12ч)                |

### Роли

| Действие | admin | read-only |
|---|:---:|:---:|
| Просмотр дашборда / таргетов / сканов / уязвимостей | ✅ | ✅ |
| Экспорт CSV/JSON | ✅ | ✅ |
| Запуск разового скана, Run now по шедулам | ✅ | ✅ |
| Acknowledge уязвимостей | ✅ | ✅ |
| Добавление / удаление таргетов, правка тегов | ✅ | ❌ |
| Создание / pause / удаление шедулов | ✅ | ❌ |
| Настройки и интеграции (Settings) | ✅ | ❌ |
| Управление пользователями (вкладка Users) | ✅ | ❌ |

Управление пользователями — вкладка **Users** в UI (только для admin) или REST:
`GET/POST /api/users`, `PATCH/DELETE /api/users/{id}`. Нельзя удалить/разжаловать
последнего активного админа и нельзя удалить собственную учётку.

Если `AUTH_ENABLED=true`, но `ADMIN_PASSWORD`/`JWT_SECRET` не заданы, вход
невозможен. Токен (с ролью в payload) хранится в `localStorage` и передаётся в
заголовке `Authorization: Bearer <token>`. Публичны только `/health`, статика SPA
и эндпоинты `/api/auth/login` и `/api/auth/config`.
